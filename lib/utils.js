 ...
 */
const getDateString = originalDate => {
 ...
 * @param  {string} props.defaultFolder - Default folder when nothing applies from rules
 * @param  {string} props.log - If present add unknown metadata to log
 * @return {string[]} Array of strings for path
 */
const getInnerPath = ({ metadata, rules, defaultFolder, log }) => {
  const rule = R.find(rule => R.whereEq(rule.metadata)(metadata), rules);
  if (!rule) {
    if (log) {
      const metadataRaw = util.inspect(metadata, { depth: null, showHidden: false });
      fs.writeFile(log, metadataRaw, { flag: 'a+' }, () => {});
    }
    if (defaultFolder) {
